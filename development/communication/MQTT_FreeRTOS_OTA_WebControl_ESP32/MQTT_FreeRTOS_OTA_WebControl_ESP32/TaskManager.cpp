...
TaskHandle_t TaskManager::createTask(TaskFunction_t taskFunction,
    ...
    if (result != pdPASS) {
        Logger::log(Logger::ERROR, String("Failed to create task: ") + taskName);
        return nullptr;
    }

    Logger::log(Logger::INFO, String("Task created: ") + taskName);
    return taskHandle;
}
...
QueueHandle_t TaskManager::createQueue(size_t queueLength, size_t itemSize) {
    ...
    if (queue == nullptr) {
        Logger::log(Logger::ERROR, "Failed to create queue");
    } else {
        Logger::log(Logger::DEBUG, "Queue created successfully");
    }
...
void TaskManager::deleteTask(TaskHandle_t& taskHandle) {
    if (taskHandle != nullptr) {
        ...
        taskHandle = nullptr;
        Logger::log(Logger::DEBUG, "Task deleted");
    }
}
...
void TaskManager::deleteQueue(QueueHandle_t& queueHandle) {
    if (queueHandle != nullptr) {
        ...
        queueHandle = nullptr;
        Logger::log(Logger::DEBUG, "Queue deleted");
    }
}
...
void TaskManager::suspendTask(TaskHandle_t taskHandle) {
    if (taskHandle != nullptr) {
        ...
        Logger::log(Logger::DEBUG, "Task suspended");
    }
}
...
void TaskManager::resumeTask(TaskHandle_t taskHandle) {
    if (taskHandle != nullptr) {
        ...
        Logger::log(Logger::DEBUG, "Task resumed");
    }
}
...
void TaskManager::checkStackUsage(TaskHandle_t taskHandle) {
    if (taskHandle != nullptr) {
        ...
        if (stackHighWaterMark < 512) {  // Seuil d'avertissement
            Logger::logWithHeap(Logger::WARNING, 
