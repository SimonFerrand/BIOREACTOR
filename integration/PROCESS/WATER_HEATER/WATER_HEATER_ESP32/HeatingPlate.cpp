...
void HeatingPlate::control(bool state, int value) {
    ...
    status = state;
    Logger::log(Logger::LogLevel::INFO, String(_name) + " control called with state: " + String(state) + " and value: " + String(value));
}

void HeatingPlate::controlRelay(bool state) {
    digitalWrite(_controlPin, state ? HIGH : LOW);
    status = state;
    Logger::log(Logger::LogLevel::INFO, String(_name) + (status ? " is ON" : " is OFF"));
}
...
void HeatingPlate::controlPWM(int value) {
    ...
    status = (pwmValue > 0);
    Logger::log(Logger::LogLevel::INFO, String(_name) + (status ? " is ON with power: " + String(value) + "%" : " is OFF"));
}
...
void HeatingPlate::controlWithPID(double pidOutput) {
    ...
    Logger::log(Logger::LogLevel::INFO, String(_name) + " PID control with power: " + String(percentPower) + "%");
}
...
void HeatingPlate::control(bool state, int value) {
    ...
    Logger::log(Logger::LogLevel::INFO, String(_name) + 
        " Control called - State: " + String(state) + 
        ", Value: " + String(value) + 
        ", Current Status: " + String(_status) + 
        ", PWM Capable: " + String(_isPWMCapable));

    if (state != _status || (state && value != _currentValue)) {
        if (state) {
            Logger::log(Logger::LogLevel::INFO, "Attempting to turn ON with value: " + String(value));
            if (_isPWMCapable) {
                controlPWM(value);
            } else {
...
void HeatingPlate::controlPWM(int value) {
    ...
    _status = (pwmValue > 0);
    Logger::log(Logger::LogLevel::INFO, String(_name) + (_status ? " is ON with power: " + String(value) + "%" : " is OFF"));
}

void HeatingPlate::controlOnOff(bool state) {
    digitalWrite(_relayPin, state ? HIGH : LOW);
    _status = state;
    //Logger::log(Logger::LogLevel::INFO, String(_name) + (_status ? " is ON" : " is OFF"));
    Logger::log(Logger::LogLevel::INFO, String(_name) + (_status ? F(" is ON") : F(" is OFF")));
}
...
void HeatingPlate::controlWithCycle(int percentage) {
    ...
    Logger::log(Logger::LogLevel::INFO, String(_name) + " Duty Cycle: " + String(_dutyCycle * 100) + "%");
}
